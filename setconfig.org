...
- Uma observação é que muitas configurações eu copiei do Github do pacote e só modifiquei o que era necessário, então recomendo conferir individualmente os pacotes e modificar o que quiser ao seu gosto. Como ainda não entendo tudo que pode oferecer, não personalizei muito.
* Index OF
...
(load-file (expand-file-name "ambiente.el" user-emacs-directory))
#+end_src
...
(use-package emacs
  ...
  (defun open-init-file-config ()
    (interactive)
    (find-file ambiente/emacs-config))
...
** Configurações do IRC
#+BEGIN_SRC emacs-lisp
(setq erc-server ambiente/irc-ip)
(setq erc-port ambiente/irc-porta)
#+END_SRC

* Configurações de pacotes
Aqui vai encontrar configurações de pacotes nativos ou instalados.
** Slack-C
Meu próprio pacote para criar projetos em C rapidamente.
#+BEGIN_SRC emacs-lisp
(use-package slack-c 
  ;;:ensure nil
  :vc (:url "https://github.com/rahvax/slack-c.el.git"
	    :rev :newest)
  :custom
  (slackc-dir-path "~/Documents/Forgejo/"))
#+END_SRC
** Latex
#+BEGIN_SRC emacs-lisp
  ...
  (org-capture-templates
   '(("p" "Pessoal")
      ...
      ("al" "Leitura" entry (file+olp "~/Documents/org/tasks.org" "Read")
       "* TOREAD %? :read: \nSCHEDULED: %^t\n%i" :empty-lines-after 1)
      ("ap" "Pesquisa" entry (file+olp "~/Documents/org/tasks.org" "Research")
       "* TODO %? :resch: \nSCHEDULED: %^t\n%i" :empty-lines-after 1)
      ("ae" "Escrever" entry (file+olp "~/Documents/org/tasks.org" "Write")
       "* TODO %? :write: \nSCHEDULED: %^t\n%i" :empty-lines-after 1)
      ("ac" "CTF" entry (file+olp "~/Documents/org/tasks.org" "CTF")
       "* TODO %? :ctf: \nSCHEDULED: %^t\n%i" :empty-lines-after 1)
  ...
  (setq org-roam-capture-templates
   '(     
     ...
     ("tc" "Code" plain "%?"
      ...
      :unarrowed t)
     ("to" "Offsec" plain "%?"
      :target (file+head "tech/offsec/${slug}.org" "#+title: OffSec: ${title}\n#+filetags: :offsec:tech:infosec:\n#+author: %n \n#+date: %U\n\n")
      :unarrowed t)
     ("tt" "Tools" plain "%?"
      :target (file+head "tech/offsec/${slug}.org" "#+title: Tools: ${title}\n#+filetags: :offsec:tech:infosec:\n#+author: %n \n#+date: %U\n\n")
      :unarrowed t)
...
(use-package dashboard
  :ensure t
  :config
  (setq dashboard-banner-logo-title ambiente/emacs-welcome)
  (setq dashboard-startup-banner ambiente/emacs-banner)
...
(use-package doom-themes
  ...
  ;; Carregar o tema e aplicar correção
